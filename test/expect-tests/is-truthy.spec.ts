import {
  ...
  export class IsTruthyTests {
      ...
      @TestCase(1, false)
      @TestCase(true, false)
      public failsOnlyWhenNotTruthy(val: any[], throws: boolean) {
      ...
      public messageAccurate(val: any[], not: boolean, msgPattern: RegExp) {
        const lambda = () => Assert(val).maybe(not).isTruthy();
